...
let aiPaddleX = (canvas.width - paddleWidth) / 2; // AI Paddle
...
function update() {
    ...
    drawPaddle(paddleX, canvas.height - paddleHeight); // User Paddle
    drawPaddle(aiPaddleX, 0); // AI Paddle
    drawBall();
    ...
    // AI Paddle Movement
    if (ballX < aiPaddleX) {
        aiPaddleX -= 5; // Move left
    } else if (ballX > aiPaddleX + paddleWidth) {
        aiPaddleX += 5; // Move right
    }

    // Keep AI paddle within canvas bounds
    aiPaddleX = Math.max(0, Math.min(aiPaddleX, canvas.width - paddleWidth));
